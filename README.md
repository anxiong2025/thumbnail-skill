...
当前版本先把一个旗舰模板做到稳定：上传真人、真实证据素材和可选风格参考，提供主题与比例，即可制作 `S11 真人高密度大字`封面。系统会主动完成标题、副标题和证据标签。`S01–S10` 暂时保留为后续路线；目录编号可省略。

> 当前版本：`v0.6.0`。“爆款”表示以提高信息流点击吸引力为目标的设计方法，不代表或保证播放量、点击率与商业结果。

## 核心能力

- 当前集中精修 1 个旗舰模板：`S11 真人高密度大字`；其余 10 类结构作为后续路线保留。
- 默认直接生成完整视觉：超大黄白粗黑描边标题、35%–55%真人抠图、真实界面背景、倾斜证据卡、大数字、箭头和绿色勾选。
...
| `S11` | <a href="demos/S11-signal-impact-6x7/preview.png"><img src="demos/S11-signal-impact-6x7/preview.png" width="180" alt="S11 真人高密度大字缩略图"></a> | 真人高密度大字 | AI 教程、产品实测、小白科普、强观点 | `$create-social-thumbnails 真人高密度大字，6:7；根据主题主动写标题，真人占约45%，使用真实界面背景、倾斜榜单卡、大数字、粗箭头和三条绿色勾选。` |
...
`S11 Creator Impact / 真人高密度大字`是本仓库当前集中精修的高密度信息流系统。核心阅读顺序是**超大标题 → 真人表情 → 大数字 → 证据卡 → 勾选项**。

- **钩子标题**：两到三行超粗大字，占竖版高度约 35%–45%，柔和浅黄 `#FDFFA7` 与白色交替，使用极粗黑描边。
- **人物轮廓**：真人占 35%–55%，位于画面下半部，使用浅黄或白色描边并自然越界；已有抠图禁止重新装框。
- **真实背景**：优先把软件界面、榜单、产品屏幕或真实场景放大、模糊、压暗作为背景，不另造无关抽象背景。
- **证明组件**：使用一张倾斜真实证据卡、一个大数字/数量、粗手绘箭头和最多三条绿色勾选项。
- **默认出图**：用户要求制作时直接生成完整封面；只有用户明确需要源文件或确定性排版时才改用代码合成。
- **失败条件**：标题太小、人物低于35%、抠图被装框、无关背景、细字体、大面积留白、证据虚构或人物身份变化，均需重做。

#### 标准示例

下图是 `S11 真人高密度大字` 的标准基准：深色真实界面背景、顶部超大黄白描边主题、右下真人抠图、左下倾斜证据卡、大数字、粗箭头和底部三条勾选结论。新主题应复现这套信息层级和密度，而不是复制其中的人物、模型、数据或文案。

<a href="demos/S11-signal-impact-6x7/preview.png"><img src="demos/S11-signal-impact-6x7/preview.png" width="420" alt="S11 真人高密度大字标准示例"></a>

字体采用双层级：**主标题与大数字可以使用粗犷手绘/马克笔字形；副标题、模型名、数据、勾选项和证据标签使用规整粗黑体。** 小字全部手绘会损害信息流缩略图的可读性。

手绘字体分层测试：<a href="demos/S11-handdrawn-type-test/preview.png"><img src="demos/S11-handdrawn-type-test/preview.png" width="260" alt="S11 主标题手绘字体测试"></a>

该测试只改变主标题和大数字的字形节奏，布局、人物、背景、证据卡与小字层级保持一致。它验证的是字体策略，不作为另一套独立风格。

完整复现步骤见 [`Creator Impact 标准 SOP`](create-social-thumbnails/references/signal-impact-system.md)。

完整设计系统见 [`signal-impact-system.md`](create-social-thumbnails/references/signal-impact-system.md)。其他风格会在这个旗舰模板形成稳定演示、复用流程和质量基线之后再继续开发。

查看 [S11 完整 Demo、复现方式与素材权利说明](demos/S11-signal-impact-6x7/README.md)。
